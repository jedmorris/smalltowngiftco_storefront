  ...
  return (
    <div className="flex gap-4 py-4">
      ...
      <Link
        ...
        className="relative w-20 h-20 flex-shrink-0 bg-stucco rounded-lg overflow-hidden"
      >
        {image ? (
          <Image
            src={image.url}
      ...
      <div className="flex-1 min-w-0">
        <Link
          href={`/products/${merchandise.product.handle}`}
          className="text-sm font-medium text-ink hover:text-apricot-deep transition-colors line-clamp-2"
        >
          {merchandise.product.title}
        </Link>
        {variantTitle && (
          <p className="text-xs text-ink-muted mt-0.5">{variantTitle}</p>
        )}
        ...
        <div className="flex items-center gap-2 mt-2">
          <div className="flex items-center border border-soft rounded">
            <button
              onClick={() =>
                item.quantity === 1
                  ? removeItem(item.id)
          ...
          <button
            ...
            disabled={isLoading}
            className="p-1 text-ink-subtle hover:text-red-500 transition-colors disabled:opacity-50"
